...
面向 A 股本地数据的 AI 因子研究工作台。`microfactor` 负责把研报、投资逻辑、因子想法，整理成可以审核、可以运行、可以落盘的标准因子代码：

```text
研报 / 想法 -> FactorSpec -> Python compute() -> Parquet 因子值
```

它和 [microshare](https://github.com/zer0quant/microshare) 是配套关系：

- `microshare`：负责本地 A 股数据采集、同步和查询
- `microfactor`：负责因子规范、因子生成、预处理、评估和存储
...
- `factor-eval-html` skill：固化因子评估 HTML 的布局、指标、图表、中文解读和归档验收规范
...
- 已经用 `microshare` 同步好的本地 A 股数据

默认要求 `microshare` 和本项目在同级目录：
...
如果你的 `microshare` 在别的位置，先改 `pyproject.toml` 里的路径：
...
```bash
git clone https://github.com/zer0quant/microfactor.git
cd microfactor
uv sync
```
...
process_universe = "hushen_mainboard_previous_day_bottom1000"
start_date       = "20160101"
end_date         = ""
```
...
因子计算和评估完成后，可以像使用 `microshare.pro_api()` 一样通过统一的只读接口访问因子数据、元数据和评估结果：
...
# 单因子长表：trade_date, ts_code, value
values = api.factor(
    "momentum",
    start_date="20240101",
    end_date="20241231",
...
# 多因子宽表：trade_date, ts_code, momentum, inverse_pb
panel = api.factor(
    ["momentum", "inverse_pb"],
    start_date="20240101",
    end_date="20241231",
...
# 通过和未通过因子都可读取；训练前可显式筛选 passed
passed = api.factor_basic(evaluation_status="passed")
latest_eval = api.factor_evaluation("momentum", run_id="latest")
```

因子数据采用不可变版本和 `latest` 指针。新增日期会幂等追加，已存在日期内容不一致时拒绝覆盖；公式、输入字段或复权模式变化必须显式发布新版本。详细字段、版本和增量规则见 [docs/factor-api.md](docs/factor-api.md)。

当前项目内置了 quant-hive 因子目录中的 65 个非 `model_*` 因子。它们的元数据快照
在 `microfactor/catalog/`，由 `config/factors.toml` 注册；股票池成员统一通过
Microshare 的 `pro.universe("hushen_mainboard_previous_day_bottom1000", ...)`
接口取得。批量评估使用固定的 1 日 `open_t1`、10 分位和 10 bps 配置：

```bash
uv run python main.py --config config/settings.toml evaluate-batch --file config/evaluation_batch.toml
```
...
因子代码只应该访问 `FactorFrame`，不要自己读文件、查 DuckDB、或者直接调用 `microshare`。字段契约和存储格式详见 [docs/factor-contract.md](docs/factor-contract.md)。
...
- [因子数据库 API](docs/factor-api.md)：因子查询、评估查询、版本和增量发布契约
