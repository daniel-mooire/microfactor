...
AI-assisted factor research workbench for local A-share data. `microfactor` turns research reports and factor ideas into reviewable, executable, persisted factor code:

```text
report / idea -> FactorSpec -> Python compute() -> Parquet factor values
```

It is built to work with [microshare](https://github.com/zer0quant/microshare):

- `microshare`: local A-share data collection, sync, and query
- `microfactor`: factor specification, generation, preprocessing, evaluation, and storage
...
- `microshare` provider that maps local market data into wide factor panels
...
- Local A-share data synced with `microshare`

By default, `microfactor` expects `microshare` next to this repo:
...
If your `microshare` checkout lives elsewhere, change the path in `pyproject.toml` first:
...
The copied catalog contains all 65 non-`model_*` quant-hive factors. They are registered
from `microfactor/catalog/` by `config/factors.toml`; universe membership is queried
through Microshare's `pro.universe` API. The standard batch uses one-day `open_t1`,
10 quantiles, and 10 bps transaction costs:
...
Factor code should only read data from `FactorFrame`. It should not read files, query DuckDB, or call `microshare` directly. See [docs/factor-contract.md](docs/factor-contract.md) for field mappings and storage layout (Chinese).
...
- Built around local A-share data and `microshare`.
