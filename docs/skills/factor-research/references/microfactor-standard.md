# microfactor Standard Factor Contract

Generated factors must target microfactor, not Qlib.
...
| `volume` | volume, mapped from microshare `vol` |
...
Factor code must not call microshare, DuckDB, parquet readers, network APIs, or
