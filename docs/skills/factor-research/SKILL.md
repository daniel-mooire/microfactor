...
description: Use when extracting, reviewing, implementing, or validating stock factors from quant research reports, formulas, or natural-language investment ideas for microfactor.
---
...
standard microfactor factors. The workflow is file-driven: every intermediate
artifact must be written to disk so the user can inspect, edit, and resume.

The implementation target is always microfactor native code:
...
- `viable`: computable from current microfactor standard fields
...
Generate a microfactor-standard `spec`, `logic_expression`, and
`implementation_plan`. `logic_expression` is metadata for review and duplicate
checking only.

Read `references/microfactor-standard.md` before this step.
...
- do not read parquet, DuckDB, paths, or microshare APIs inside the factor
...
- `references/microfactor-standard.md`: factor interface and data contract
