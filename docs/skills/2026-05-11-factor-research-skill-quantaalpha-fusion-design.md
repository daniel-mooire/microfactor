...
**目标：** 从研报或研究方向中提取、筛选、规范化、实现和归档可计算因子，保留人工检查点与断点续跑，同时吸收 QuantaAlpha 的假设生成、质量门和反馈能力，但将因子实现规范从 Qlib 表达式改为 microfactor/microshare 原生规范。
...
microfactor 规范提供数据契约、窗口契约和代码接口
```
...
QuantaAlpha 的原始因子生成遵循 Qlib 表达式规范，例如 `$close`、`TS_MEAN()`、`RANK()`。本 skill 不直接沿用该规范，因为 microfactor 的实际数据来自 microshare，本地字段、复权方式、日期格式和存储方式都不同。
...
因子代码不能直接读取 parquet、DuckDB，也不能直接调用 `microshare.pro.daily()`。所有数据访问都通过标准 Provider 完成。
...
`MicroshareDataProvider` 负责把 microshare 字段映射到标准字段：
...
主流程只纳入 factor 研究相关 prompt，并用 microfactor 标准因子规范包裹 QuantaAlpha 的生成约束。

| 分层 | QuantaAlpha 文件 | 用途 |
|---|---|---|
| 研报加载 | `quantaalpha/factors/loader/prompts.yaml` | 分类、提取、公式、相关性、可行性、候选去重 |
| 假设与规范化 | `quantaalpha/factors/prompts/prompts.yaml` | 研究方向转假设、假设转因子实现计划；借鉴表达式约束思想但替换为 microfactor 规范 |
...
把投资假设转换为 microfactor 标准因子规范，包括 `FactorSpec`、`logic_expression` 和实现计划。
...
在 QuantaAlpha `hypothesis2experiment` prompt 外层增加 microfactor 约束：

```text
Generate a microfactor-standard factor, not a Qlib expression.
...
这是 microfactor 新增质量门，用于替代 Qlib 表达式可解析性检查。
...
6. 因子逻辑是否直接依赖 microshare API 或 parquet 路径
```
...
在 prompt 外层增加 microfactor 代码接口约束：

```text
Generate Python code for microfactor, not Qlib.
...
- Do not read files, parquet, DuckDB, or call microshare APIs inside the factor.
...
| 规范生成 | `hypothesis2experiment` | `factor_experiment_output_format` 经 microfactor 外层约束改写 |
| 数据/算子约束 | microfactor 标准因子规范 | `function_lib_description` 仅作约束思路参考 |
...
3. 最后接入执行后端：`MicroshareDataProvider -> FactorFrame -> compute() -> FactorStorage.write()`。
