...
**Architecture:** Extend the standard `FactorFrame` field contract with `total_mv` and `circ_mv`, loading those fields from `microshare.daily_basic` while keeping OHLCV fields on `pro_bar`. Add dedicated market-cap factor classes and a small market-cap storage helper used by a new `compute-market-cap` CLI command.

**Tech Stack:** Python 3.11, pandas, numpy, click, pytest, existing microfactor `Factor`, `FactorFrame`, `FactorStorage`, and `FactorPreprocessPipeline`.

---

## File Structure

- Modify `microfactor/core/__init__.py`: add `total_mv` and `circ_mv` standard fields; route market-cap fields through `daily_basic`.
- Create `microfactor/factors/market_cap.py`: `LogTotalMarketCap` and `LogCirculatingMarketCap`.
- Modify `microfactor/factors/__init__.py`: export market-cap factor classes.
...
**Files:**
- Modify: `microfactor/core/__init__.py`
...
```python
def test_microshare_provider_loads_market_cap_fields_from_daily_basic():
    pro = FakeLocalPro()
    provider = MicroshareDataProvider(pro)
...
def test_microshare_provider_combines_price_and_market_cap_fields():
    pro = FakeLocalPro()
    provider = MicroshareDataProvider(pro)
...
```bash
git add microfactor/core/__init__.py tests/test_factor_standard.py
...
**Files:**
- Create: `microfactor/factors/market_cap.py`
...
from microfactor.factors import LogCirculatingMarketCap, LogTotalMarketCap
...
```bash
git add microfactor/factors/market_cap.py microfactor/factors/__init__.py tests/test_market_cap_factors.py
...
```python
from microfactor.core import Factor, MicroshareDataProvider, run_factor, to_factor_output
from microfactor.factors import (
    DailyReturn,
    IntradayReturn,
    LogCirculatingMarketCap,
    LogTotalMarketCap,
...
def compute_and_store_market_cap_factors(
    ...
    provider: MicroshareDataProvider,
...
def compute_market_cap(ctx):
    ...
    provider = MicroshareDataProvider(LocalPro(cfg.microshare_data_dir))
...
```bash
uv run ruff check main.py microfactor/core/__init__.py microfactor/factors tests/test_factor_standard.py tests/test_market_cap_factors.py tests/test_main.py
```
