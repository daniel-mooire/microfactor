...
**Goal:** 为 microfactor 的 preprocess 和 evaluate-batch 流程添加飞书富文本卡片通知。

**Architecture:** 新增 `microfactor/notify/` 包，包含 `NullNotifier`（no-op）和 `FeishuNotifier`（发飞书卡片）；`load_notifier(config)` 工厂函数根据配置返回对应实例；pipeline 函数通过可选参数接收 notifier，`None` 时静默。
...
| 操作 | 路径 | 职责 |
|------|------|------|
| 创建 | `microfactor/notify/__init__.py` | 导出 FeishuNotifier, NullNotifier, load_notifier |
| 创建 | `microfactor/notify/null.py` | NullNotifier（5 个空方法） |
| 创建 | `microfactor/notify/feishu.py` | FeishuNotifier + 卡片构建 + HTTP 发送 |
...
| 修改 | `microfactor/config.py` | 增加 `notify_webhook_url: str` 字段 |
| 修改 | `config/settings.toml` | 增加 `[notify]` 节 |
| 修改 | `tests/test_config.py` | 补充 notify_webhook_url 测试 |
| 修改 | `microfactor/pipeline.py` | `run_build_stage()` 加 notifier 参数 |
| 修改 | `microfactor/eval/pipeline.py` | `evaluate_factors()` 加 notifier 参数 |
...
**Files:**
- Create: `microfactor/notify/null.py`
- Create: `microfactor/notify/__init__.py`（空壳，Task 5 填充）
...
```bash
mkdir -p microfactor/notify tests/notify
touch microfactor/notify/__init__.py tests/notify/__init__.py
```
...
```bash
git add microfactor/notify/__init__.py microfactor/notify/null.py tests/notify/__init__.py tests/notify/test_null.py
...
def load_config(path: Path = Path("config/settings.toml")) -> Config:
    ...
    try:
        return Config(
            microshare_data_dir=Path(raw["microshare"]["data_dir"]),
...
```bash
git add microfactor/config.py config/settings.toml tests/test_config.py
...
**Files:**
- Create: `microfactor/notify/feishu.py`（只实现 `_build_card`）
...
def test_build_card_structure():
    n = _make_notifier()
    card = n._build_card(
        title="[microfactor] `raw` 完成 ✅",
        color="green",
        fields=[("因子数", "40"), ("耗时", "5.3s")],
    )
    assert card["msg_type"] == "interactive"
    c = card["card"]
    assert c["header"]["title"]["content"] == "[microfactor] `raw` 完成 ✅"
...
class FeishuNotifier:
    def __init__(self, webhook_url: str, *, app: str = "microfactor") -> None:
...
```bash
git add microfactor/notify/feishu.py tests/notify/test_feishu.py
...
**Files:**
- Modify: `microfactor/notify/feishu.py`（实现 `_send` + 5 个公开方法）
...
将 `microfactor/notify/feishu.py` 中 `_send` 和公开方法替换为：
...
**Files:**
- Modify: `microfactor/notify/__init__.py`
- Modify: `tests/notify/test_null.py`（追加 load_notifier 测试）

- [ ] **Step 1: 在 tests/notify/test_null.py 末尾追加测试**

```python
from microfactor.config import Config
from microfactor.notify import load_notifier, FeishuNotifier, NullNotifier
from pathlib import Path
...
```python
# microfactor/notify/__init__.py
from __future__ import annotations
...
```bash
git add microfactor/notify/__init__.py tests/notify/test_null.py
...
**Files:**
- Modify: `microfactor/pipeline.py`（`run_build_stage` 加 notifier 参数）
...
def test_run_build_stage_calls_notify_start_and_done(tmp_path):
    ...
    from microfactor.pipeline import run_build_stage
...
- [ ] **Step 3: 修改 microfactor/pipeline.py 中的 run_build_stage**
...
```bash
git add microfactor/pipeline.py tests/test_pipeline_notify.py
...
**Files:**
- Modify: `microfactor/eval/pipeline.py`（`evaluate_factors` 加 notifier 参数和进度里程碑）
...
def test_evaluate_factors_calls_notify_start_and_eval_done(tmp_path, monkeypatch):
    from microfactor.eval.pipeline import evaluate_factors
    ...
    monkeypatch.setattr(
        "microfactor.eval.pipeline.evaluate_factor", lambda **kwargs: stub_result
    )
    monkeypatch.setattr(
        "microfactor.eval.pipeline.write_run_summary",
        lambda **kwargs: {"metadata": tmp_path / "meta.json"},
    )
    monkeypatch.setattr(
        "microfactor.eval.pipeline.create_run_directory",
        lambda *args, **kwargs: ("20260614_120000", tmp_path),
    )
    monkeypatch.setattr(
        "microfactor.eval.pipeline.load_price_data", lambda *args, **kwargs: pd.DataFrame()
    )
    monkeypatch.setattr(
        "microfactor.eval.pipeline.load_universe_panel", lambda *args, **kwargs: None
    )
...
- [ ] **Step 3: 修改 microfactor/eval/pipeline.py**
...
```bash
git add microfactor/eval/pipeline.py tests/test_eval_pipeline_notify.py
...
找到 `from microfactor.config import load_config` 所在行，在其下方添加：

```python
from microfactor.notify import load_notifier
```
...
```python
    ...
    pro = LocalPro(cfg.microshare_data_dir) if stage in {"preprocess", "all"} else None
