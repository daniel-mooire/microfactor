...
**Architecture:** Add a focused `microfactor/eval/` package with config/result dataclasses, loaders/adapters, metrics, plots, artifacts, and orchestration. Keep `main.py` as a thin CLI wrapper that constructs `FactorStorage`, `LocalPro`, and `EvaluationConfig`, then calls the eval API.

**Tech Stack:** Python 3.11, pandas, alphalens-reloaded, matplotlib, click, pytest, pyarrow/parquet, existing `FactorStorage` and microshare `LocalPro`.
...
- `microfactor/eval/config.py`: dataclasses and config validation.
- `microfactor/eval/alphalens_adapter.py`: factor, price, and universe format conversions.
- `microfactor/eval/loaders.py`: stored factor, price, and universe loading helpers.
- `microfactor/eval/metrics.py`: Alphalens metric wrappers and summary table builder.
- `microfactor/eval/plots.py`: PNG figure writers.
- `microfactor/eval/artifacts.py`: run directory creation and artifact writes.
- `microfactor/eval/pipeline.py`: single-factor and multi-factor orchestration.
...
- `microfactor/eval/__init__.py`: export public API.
...
- Create: `microfactor/eval/config.py`
...
from microfactor.eval import EvaluationConfig
...
```python
from microfactor.eval.config import (
    EvaluationConfig,
    EvaluationRunResult,
...
```bash
git add microfactor/eval/__init__.py microfactor/eval/config.py tests/test_eval_config.py
...
- Create: `microfactor/eval/alphalens_adapter.py`
...
Expected: FAIL because `microfactor.eval.alphalens_adapter` does not exist.
...
from microfactor.eval.config import ReturnType
...
```bash
git add microfactor/eval/alphalens_adapter.py tests/test_eval_alphalens_adapter.py
...
- Create: `microfactor/eval/metrics.py`
...
from microfactor.eval.metrics import build_summary, calculate_long_short_spread
...
Expected: FAIL because `microfactor.eval.metrics` does not exist.
...
```bash
git add microfactor/eval/metrics.py tests/test_eval_metrics.py
...
- Create: `microfactor/eval/artifacts.py`
- Create: `microfactor/eval/plots.py`
...
from microfactor.eval.artifacts import (
    create_run_directory,
    write_factor_artifacts,
    write_run_summary,
)
from microfactor.eval.config import EvaluationConfig
...
```bash
git add microfactor/eval/artifacts.py microfactor/eval/plots.py tests/test_eval_artifacts.py
...
- Create: `microfactor/eval/loaders.py`
- Create: `microfactor/eval/pipeline.py`
- Modify: `microfactor/eval/__init__.py`
...
from microfactor.eval import EvaluationConfig, evaluate_factors
...
def test_evaluate_factors_writes_run_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "microfactor.eval.pipeline.get_clean_factor_and_forward_returns",
...
from microfactor.eval.loaders import (
    load_price_data,
    load_stored_factor,
    load_universe_panel,
)
...
from microfactor.eval.pipeline import evaluate_factor, evaluate_factors
...
```bash
git add microfactor/eval/loaders.py microfactor/eval/pipeline.py microfactor/eval/__init__.py tests/test_eval_pipeline.py
...
def _run_evaluation_command(
    ...
    *,
    ...
    result = evaluate_factors(
        factor_names=factor_names,
        storage=storage,
        pro=LocalPro(cfg.microshare_data_dir),
        config=config,
    )
    logger.info(
...
```bash
uv run ruff check main.py microfactor/eval tests/test_eval_config.py tests/test_eval_alphalens_adapter.py tests/test_eval_metrics.py tests/test_eval_artifacts.py tests/test_eval_pipeline.py tests/test_main.py
```
...
```bash
git diff --stat
git diff -- microfactor/eval main.py tests/test_eval_config.py tests/test_eval_alphalens_adapter.py tests/test_eval_metrics.py tests/test_eval_artifacts.py tests/test_eval_pipeline.py tests/test_main.py
```
...
```bash
git add microfactor/eval main.py tests/test_eval_config.py tests/test_eval_alphalens_adapter.py tests/test_eval_metrics.py tests/test_eval_artifacts.py tests/test_eval_pipeline.py tests/test_main.py
