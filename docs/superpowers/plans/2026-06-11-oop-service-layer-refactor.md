...
**Architecture:** `main.py` becomes a 5-line entry point delegating to a `microfactor/cli/` package. Business orchestration moves into `microfactor/services/` classes that receive their dependencies (storage, data provider) via constructor injection. An `AppContext` composition root builds those dependencies from config. `typing.Protocol` interfaces decouple services from `microshare`. The domain layer (`core`, `preprocess`, `eval`, `storage`, `registry`) is untouched except for one deduplication.
...
|---|---|
| `_parse_trade_dates`, `_factor_long_to_wide`, `_factor_wide_to_long`, `_filter_long_by_universe`, `_filter_panel_by_universe`, `read_universe_panel` (396–470) | `microfactor/panel.py` (Task 1) |
| `configure_logging`, `LOG_FORMAT` (51, 202–213) | `microfactor/context.py` (Task 4) |
| `RETURN_FACTORS`, `MARKET_CAP_FACTORS` (32–41) | `microfactor/factors/builtin.py` (Task 5) |
| `compute_and_store_factors`, `compute_and_store_market_cap_factors` (227–310) | merged into `microfactor/services/compute.py` (Task 6) |
| `standardize_stored_factor`, `neutralize_stored_factor`, `preprocess_stored_factor`, `standardize_stored_panel`, `_standardized_factor_name`, `MARKET_CAP_PREPROCESS_CONFIG`, `STANDARD_PREPROCESS_CONFIG`, `NEUTRALIZATION_SIZE_FACTOR` (42–50, 313–393) | `microfactor/services/preprocess.py` (Task 7) |
| `_run_evaluation_command`, `_run_evaluation_job` (661–745) | `microfactor/services/evaluate.py` + `microfactor/cli/evaluate_cmds.py` (Tasks 8, 9) |
| All `@cli.command` functions | `microfactor/cli/` package (Task 9) |

New abstractions: `microfactor/core/protocols.py` (Task 2), `microfactor/naming.py` (Task 3), `microfactor/context.py` (Task 4).
...
### Task 1: Panel utilities module (`microfactor/panel.py`)

**Files:**
- Create: `microfactor/panel.py`
- Create: `tests/test_panel.py`
- Modify: `main.py` (delete lines 396–470, update imports/callers)
- Modify: `microfactor/preprocess/pipeline.py` (replace private `_parse_trade_dates`)
...
Expected: FAIL — `ModuleNotFoundError: No module named 'microfactor.panel'`

- [ ] **Step 3: Implement `microfactor/panel.py`**
...
from microfactor.core import to_factor_output
...
Note `wide_to_long` is `to_factor_output` — `main.py:464-470`'s `_factor_wide_to_long` was a byte-for-byte duplicate of the DataFrame branch of `to_factor_output` in `microfactor/core/__init__.py:102`.
...
In `main.py`:
1. Add import: `from microfactor.panel import filter_long_by_universe, filter_panel_by_universe, long_to_wide, parse_trade_dates, read_universe_panel, wide_to_long`
...
- [ ] **Step 6: Dedupe `microfactor/preprocess/pipeline.py`**

Delete its private `_parse_trade_dates` (lines 127–130), add `from microfactor.panel import parse_trade_dates`, and rename the two call sites in `_to_wide` from `_parse_trade_dates` to `parse_trade_dates`. (No import cycle: `panel` imports `microfactor.core` only.)
...
```bash
git add microfactor/panel.py tests/test_panel.py main.py microfactor/preprocess/pipeline.py
git commit -m "refactor: extract shared panel utilities into microfactor.panel"
```

---

### Task 2: Data-source protocols (`microfactor/core/protocols.py`)

**Files:**
- Create: `microfactor/core/protocols.py`
...
Expected: FAIL — `ModuleNotFoundError: No module named 'microfactor.core.protocols'`

- [ ] **Step 3: Implement `microfactor/core/protocols.py`**
...
(`microfactor/core/__init__.py` is NOT modified — `protocols` is a submodule of the existing `core` package directory.)

- [ ] **Step 4: Run tests, type the existing seams**

Run: `uv run pytest tests/test_protocols.py -v` — expected: 2 PASS.

Then annotate `microfactor/panel.py`'s `read_universe_panel` first parameter: `pro: UniverseSource` (add `from microfactor.core.protocols import UniverseSource`), and `microfactor/exposures.py`'s `build_sw_l1_industry_panel` first parameter: `pro: IndustrySource` (same import). Run `uv run pytest` — all PASS.

- [ ] **Step 5: Commit**

```bash
git add microfactor/core/protocols.py tests/test_protocols.py microfactor/panel.py microfactor/exposures.py
git commit -m "refactor: add DataProvider/UniverseSource/IndustrySource protocols"
```

---

### Task 3: Factor naming value object (`microfactor/naming.py`)

**Files:**
- Create: `microfactor/naming.py`
...
Expected: FAIL — `ModuleNotFoundError: No module named 'microfactor.naming'`

- [ ] **Step 3: Implement `microfactor/naming.py`**
...
```bash
git add microfactor/naming.py tests/test_naming.py
git commit -m "refactor: add FactorName value object for derived-factor naming"
```

---

### Task 4: Composition root (`microfactor/context.py`)

**Files:**
- Create: `microfactor/context.py`
...
from microfactor.context import AppContext
...
Expected: FAIL — `ModuleNotFoundError: No module named 'microfactor.context'`

- [ ] **Step 3: Implement `microfactor/context.py`**

`configure_logging` body is a verbatim move from `main.py:202-213`; `pro`/`provider` keep the lazy `microshare` import that commands use today:
...
pro = LocalPro(cfg.microshare_data_dir)            # where present
provider = MicroshareDataProvider(LocalPro(...))   # where present
...
Affected commands: `status`, `factor_list_command`, `factor_info_command`, `compute_returns`, `compute_market_cap`, `standardize_factor`, `neutralize_factor`, `_run_evaluation_job`. Delete `configure_logging`, `LOG_FORMAT`, the now-unused `import sys`, the `from loguru import logger` stays (commands still log). Delete the `from microshare.api import LocalPro` lines inside command bodies and the `MicroshareDataProvider` import if no longer referenced.
...
```bash
git add microfactor/context.py tests/test_context.py main.py
git commit -m "refactor: add AppContext composition root, remove per-command wiring"
```

---

### Task 5: Built-in factor groups (`microfactor/factors/builtin.py`)

**Files:**
- Create: `microfactor/factors/builtin.py`
- Modify: `main.py` (replace tuple definitions with import)
- Modify: `tests/test_main.py` (import from new location)

- [ ] **Step 1: Implement `microfactor/factors/builtin.py`**
...
In `main.py`: delete the `RETURN_FACTORS` / `MARKET_CAP_FACTORS` definitions (lines 32–41) and add `from microfactor.factors.builtin import MARKET_CAP_FACTORS, RETURN_FACTORS`.

In `tests/test_main.py`: remove `MARKET_CAP_FACTORS, RETURN_FACTORS` from the `from main import (...)` block and add `from microfactor.factors.builtin import MARKET_CAP_FACTORS, RETURN_FACTORS`.
...
```bash
git add microfactor/factors/builtin.py main.py tests/test_main.py
git commit -m "refactor: move built-in factor groups into microfactor.factors.builtin"
```

---

### Task 6: Compute service (`microfactor/services/compute.py`)
...
**Files:**
- Create: `microfactor/services/__init__.py`
- Create: `microfactor/services/compute.py`
...
from microfactor.services.compute import FactorComputeService, ZScorePostProcess
from microfactor.storage import FactorStorage
...
Expected: FAIL — `ModuleNotFoundError: No module named 'microfactor.services'`

- [ ] **Step 3: Implement the service**

Create empty `microfactor/services/__init__.py` (docstring only: `"""Application services orchestrating domain modules."""`).
...
2. Add `from microfactor.services.compute import FactorComputeService, ZScorePostProcess`.
...
```bash
git add microfactor/services/ tests/test_services_compute.py main.py tests/test_main.py
git commit -m "refactor: merge factor computation into FactorComputeService"
```

---

### Task 7: Preprocess service (`microfactor/services/preprocess.py`)

**Files:**
- Create: `microfactor/services/preprocess.py`
...
from microfactor.panel import read_universe_panel
from microfactor.services.preprocess import FactorPreprocessService
from microfactor.storage import FactorStorage
...
- [ ] **Step 4: Implement `microfactor/services/preprocess.py`**
...
from microfactor.core.protocols import IndustrySource
from microfactor.exposures import build_sw_l1_industry_panel
from microfactor.naming import FactorName
from microfactor.panel import (
    filter_long_by_universe,
    filter_panel_by_universe,
...
2. Add `from microfactor.services.preprocess import NEUTRALIZATION_SIZE_FACTOR, FactorPreprocessService` and `from microfactor.naming import FactorName`.
...
```bash
git add microfactor/services/preprocess.py tests/test_services_preprocess.py main.py tests/test_main.py
git commit -m "refactor: extract FactorPreprocessService for standardize/neutralize"
```

---

### Task 8: Evaluation service (`microfactor/services/evaluate.py`)

**Files:**
- Create: `microfactor/services/evaluate.py`
- Modify: `main.py` (`_run_evaluation_job` shrinks to config-building + service call)

- [ ] **Step 1: Implement `microfactor/services/evaluate.py`**
...
from microfactor.eval import EvaluationConfig, EvaluationRunResult, evaluate_factors
from microfactor.storage import FactorStorage
...
(The trailing `logger.info("factor_evaluation_job_finished ...")` and `return result` stay unchanged.) Add `from microfactor.services.evaluate import EvaluationService`; remove `evaluate_factors` from the `microfactor.eval` import block in `main.py` and the now-dead `from microshare.api import LocalPro` inside the function.

- [ ] **Step 3: Run the full suite and commit**

Run: `uv run pytest` — all PASS (CLI evaluation tests in test_main.py cover this path; check how they monkeypatch `LocalPro` — if they patch `main.LocalPro` or `microshare.api.LocalPro`, the lazy import in `AppContext.pro` resolves through `microshare.api`, so patching `microshare.api.LocalPro` keeps working; adjust the monkeypatch target in the test if it patched a `main` attribute).

```bash
git add microfactor/services/evaluate.py main.py tests/test_main.py
git commit -m "refactor: extract EvaluationService facade"
```

---

### Task 9: CLI package split (`microfactor/cli/`), thin `main.py`
...
**Files:**
- Create: `microfactor/cli/__init__.py`
- Create: `microfactor/cli/root.py` — `cli` group + `status`
- Create: `microfactor/cli/registry_cmds.py` — `factor-list`, `factor-info`
- Create: `microfactor/cli/compute_cmds.py` — `compute-returns`, `compute-market-cap`
- Create: `microfactor/cli/preprocess_cmds.py` — `standardize-factor`, `neutralize-factor`
- Create: `microfactor/cli/evaluate_cmds.py` — `_parse_periods`, `_run_evaluation_command`, `_run_evaluation_job`, `evaluate-factor`, `evaluate-factors`, `evaluate-batch`, `evaluate-summary`, `show-summary`
- Rewrite: `main.py`
- Rename: `tests/test_main.py` → `tests/test_cli.py`

- [ ] **Step 1: Create `microfactor/cli/root.py`**
...
Each module starts with `from microfactor.cli.root import cli` and registers commands via the existing `@cli.command(...)` decorators. Move each command function verbatim from `main.py` (bodies already use `AppContext`/services after Tasks 4–8):
...
- `evaluate_cmds.py`: `_parse_periods`, `_run_evaluation_command`, `_run_evaluation_job`, `evaluate_factor_command`, `evaluate_factors_command`, `evaluate_batch_command`, `evaluate_summary_command`, `show_summary_command`. Imports: `click`, `pandas as pd`, `Path`, `logger`, `AppContext`, `EvaluationService`, and from `microfactor.eval`: `EvaluationConfig`, `ReportThresholds`, `find_latest_run_dir`, `generate_evaluation_report`, `load_batch_evaluation_config`.

- [ ] **Step 3: Create `microfactor/cli/__init__.py`**
...
from microfactor.cli import (  # noqa: F401  (imports register commands)
    compute_cmds,
    evaluate_cmds,
    preprocess_cmds,
    registry_cmds,
...
```python
from microfactor.cli import cli
...
In `tests/test_cli.py`, the remaining import from `main` is only `cli` — replace `from main import (cli,)` with `from microfactor.cli import cli`. If any test monkeypatches `main.<attr>`, retarget to the module that now owns the attribute (e.g. `microfactor.cli.evaluate_cmds`).
...
```bash
uv run ruff check .
git add main.py microfactor/cli/ tests/test_cli.py
git commit -m "refactor: split CLI into microfactor.cli package, main.py is a thin entry"
```
...
- Moving `MicroshareDataProvider` out of `core/__init__.py` — cosmetic, adds import-cycle risk, defer.
...
- Protocol-typing the evaluation `pro` — `eval/loaders.py` uses a wider microshare surface; type it when `eval` itself is next touched.
