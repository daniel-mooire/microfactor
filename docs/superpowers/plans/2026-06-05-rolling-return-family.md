...
**Architecture:** Add a focused `microfactor.factors.rolling_returns` module for rolling return names and raw derivation from `FactorStorage`. Add `microfactor.pipeline` for `FactorFamily`, `PreprocessProfile`, raw/preprocess orchestration, and registry entry generation. Wire a new `main.py build-factors` CLI to the pipeline while keeping existing single-factor preprocessing and evaluation commands unchanged.

**Tech Stack:** Python 3.11, pandas, Click, pytest, existing `microfactor` storage/preprocess/exposure/registry utilities, local `microshare` for universe and industry panels.

---

## File Structure

- Create `microfactor/factors/rolling_returns.py`
...
- Create `microfactor/pipeline.py`
...
- Modify `microfactor/factors/__init__.py`
  - Re-export rolling return helpers that are useful to tests and future callers.
- Modify `main.py`
  - Add the `build-factors` Click command and route it to `microfactor.pipeline.run_build_stage`.
...
from microfactor.factors.rolling_returns import (
    BASE_RETURN_FACTORS,
    WINDOWS,
    parse_rolling_return_name,
    raw_factor_names,
)
from microfactor.pipeline import get_family
...
Expected: FAIL because `microfactor.factors.rolling_returns` and `microfactor.pipeline` do not exist.
...
from microfactor.factors.rolling_returns import raw_factor_names
...
Modify `microfactor/factors/__init__.py` by adding:
...
```bash
git add microfactor/factors/rolling_returns.py microfactor/pipeline.py microfactor/factors/__init__.py tests/test_rolling_return_family.py
...
**Files:**
- Modify: `microfactor/pipeline.py`
...
from microfactor.pipeline import (
    _long_to_wide,
...
Modify `microfactor/pipeline.py` by adding imports:
...
```bash
git add microfactor/pipeline.py tests/test_build_rolling_return_factors.py
...
```python
from microfactor.pipeline import SIZE_FACTOR_NAME, preprocess_one_factor
...
Modify `microfactor/pipeline.py` imports:

```python
from microfactor.preprocess import impute_missing, neutralize, standardize, winsorize
```
...
def test_run_build_stage_raw_dispatches_raw_builder(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("microfactor.pipeline.compute_raw_rolling_return_factors", fake_raw)
...
def test_run_build_stage_all_runs_raw_then_preprocess(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("microfactor.pipeline.compute_raw_rolling_return_factors", fake_raw)
    monkeypatch.setattr("microfactor.pipeline.preprocess_all_factors", fake_preprocess_all)
...
from microfactor.pipeline import update_factor_registry
...
```bash
git add microfactor/pipeline.py config/factors.example.toml tests/test_build_rolling_return_factors.py
...
```python
from microfactor.pipeline import run_build_stage, update_factor_registry
```
...
```bash
PYTHONDONTWRITEBYTECODE=1 uv run ruff check microfactor tests main.py
```
