...
In `microfactor/storage.py`, add the import:
...
```python
from microfactor.pipeline import FAMILIES
...
In `microfactor/pipeline.py`, add imports:
...
def test_preprocess_all_factors_parallel_matches_serial(tmp_path) -> None:
    from microfactor.pipeline import preprocess_all_factors
...
In `microfactor/pipeline.py`, add `workers: int = 1` to `preprocess_all_factors` and dispatch after the industry panel is built (universe/panel loading stays shared between both paths):
...
```bash
git add microfactor/pipeline.py main.py tests/test_main.py
...
Run: `PYTHONDONTWRITEBYTECODE=1 uv run ruff check microfactor/storage.py microfactor/pipeline.py main.py tests/test_storage.py tests/test_build_rolling_return_factors.py tests/test_main.py`
