...
|------|--------|---------------|
| `microfactor/storage.py` | Modify | Add `FactorStats` dataclass + `factor_stats()` method |
| `microfactor/registry.py` | Create | `EvaluateMeta`, `FactorMeta`, `RegistryValidation`, `FactorRegistry` |
| `microfactor/eval/batch.py` | Modify | Registry-mode `factor_source` in `load_batch_evaluation_config` |
...
```python
from microfactor.storage import FactorStats, FactorStorage
...
At the top of `microfactor/storage.py`, after the imports, add:
...
```bash
git add microfactor/storage.py tests/test_storage.py
git commit -m "feat: add FactorStats and factor_stats() to FactorStorage"
```

---

## Task 2: `microfactor/registry.py` — data models + load/filter

**Files:**
- Create: `microfactor/registry.py`
...
from microfactor.registry import EvaluateMeta, FactorMeta, FactorRegistry
...
Expected: FAIL — `ModuleNotFoundError: No module named 'microfactor.registry'`

- [ ] **Step 3: Create `microfactor/registry.py`**
...
```bash
git add microfactor/registry.py tests/test_registry.py
...
```python
import pandas as pd
from microfactor.registry import RegistryValidation
...
from microfactor.eval.batch import load_batch_evaluation_config
...
- [ ] **Step 3: Update `microfactor/eval/batch.py`**
...
```bash
git add microfactor/eval/batch.py tests/test_eval_batch.py
