...
**Goal:** Rebuild the `microfactor` evaluation subsystem around explicit domain objects covering factor selection, evaluation execution, artifacts, ranked reporting, and family analysis.

**Architecture:** Introduce an OO layer under `microfactor/eval/` while preserving the current artifact layout and metric formulas. `EvaluationWorkflow` coordinates the full use case; executors handle serial/parallel strategy; `FactorEvaluator` owns single-factor evaluation; reporter and analyzer are first-class post-run components.

**Tech Stack:** Python 3.11+, pandas, Alphalens, matplotlib Agg, click, pytest, existing `FactorStorage` and microshare-like provider APIs.
...
- `microfactor/eval/domain.py`: run config, run identity, request/result dataclasses, path helpers.
- `microfactor/eval/selection.py`: explicit and registry-backed factor selection.
- `microfactor/eval/data.py`: factor, price, universe, benchmark, and max-date loading.
- `microfactor/eval/artifacts.py`: refactor existing artifact functions into `EvaluationArtifactStore` while keeping small function wrappers during migration.
- `microfactor/eval/figures.py`: `FactorFigureWriter` wrapper around existing plot functions.
- `microfactor/eval/calculator.py`: `MetricsCalculator` facade around existing metric functions.
- `microfactor/eval/evaluator.py`: `FactorEvaluator` for one factor.
- `microfactor/eval/execution.py`: serial and process-pool executors.
- `microfactor/eval/reporting.py`: `EvaluationReporter`, `SummaryRanker`, `QuantileMonotonicityLoader`, `MarkdownReportRenderer`.
- `microfactor/eval/analysis.py`: adapt current analyzer into `FamilyAnalyzer` and `EvaluationAnalysisRunner`.
- `microfactor/eval/workflow.py`: full evaluation workflow.
- `microfactor/eval/__init__.py`: export new primary API.
- `microfactor/services/evaluate.py`: delegate to workflow.
- `microfactor/cli/evaluate_cmds.py`: turn commands into thin workflow adapters.

Keep these modules as low-level function libraries:

- `microfactor/eval/alphalens_adapter.py`
- `microfactor/eval/plots.py`
- `microfactor/eval/metrics/*`
...
**Files:**
- Create: `microfactor/eval/domain.py`
- Modify: `microfactor/eval/__init__.py`
...
Expected: fail with `ModuleNotFoundError` or import error for `microfactor.eval.domain`.

- [ ] **Step 3: Implement `microfactor/eval/domain.py`**
...
from microfactor.eval.report import ReportThresholds
...
Modify `microfactor/eval/__init__.py` to import and include these names in `__all__`:
...
```bash
git add microfactor/eval/domain.py microfactor/eval/__init__.py tests/test_eval_domain.py
...
**Files:**
- Create: `microfactor/eval/selection.py`
...
from microfactor.eval.domain import EvaluationRequest
from microfactor.eval.selection import FactorSelector
...
Expected: fail because `microfactor.eval.selection` does not exist.
...
from microfactor.eval.domain import EvaluationRequest
from microfactor.registry import FactorRegistry
...
```bash
git add microfactor/eval/selection.py tests/test_eval_selection.py
...
**Files:**
- Create: `microfactor/eval/data.py`
- Modify: `microfactor/eval/loaders.py`
...
Create `microfactor/eval/data.py` by moving behavior from `loaders.py` and
`pipeline.py`:
...
Modify `microfactor/eval/loaders.py` so existing tests and imports keep working:
...
```bash
git add microfactor/eval/data.py microfactor/eval/loaders.py tests/test_eval_data.py
...
**Files:**
- Create: `microfactor/eval/figures.py`
- Create: `microfactor/eval/calculator.py`
- Modify: `microfactor/eval/artifacts.py`
...
from microfactor.eval.calculator import MetricsCalculator
from microfactor.eval.domain import EvaluationRun, EvaluationRunConfig, FactorEvaluationResult
...
def test_metrics_calculator_builds_period_sample_counts(monkeypatch):
    ...
    monkeypatch.setattr(
        "microfactor.eval.calculator.get_clean_factor_and_forward_returns",
        fake_clean,
    )
...
Add `EvaluationArtifactStore` to `microfactor/eval/artifacts.py`, keeping existing
...
from microfactor.eval.metrics import (
    build_summary,
    calculate_daily_ic,
...
```bash
git add microfactor/eval/artifacts.py microfactor/eval/figures.py microfactor/eval/calculator.py tests/test_eval_components.py
...
**Files:**
- Create: `microfactor/eval/evaluator.py`
- Modify: `microfactor/eval/pipeline.py`
...
from microfactor.eval.calculator import MetricsCalculator
from microfactor.eval.data import EvaluationDataLoader
from microfactor.eval.domain import EvaluationRun, EvaluationRunConfig
from microfactor.eval.evaluator import FactorEvaluator
...
Create `microfactor/eval/evaluator.py` by moving single-factor logic from
...
class FactorEvaluator:
    ...
    def evaluate(
        ...
        from microfactor.eval.domain import FactorEvaluationResult
...
Modify `microfactor/eval/pipeline.py` so `evaluate_factor` delegates to
...
```bash
git add microfactor/eval/evaluator.py microfactor/eval/pipeline.py tests/test_eval_evaluator.py
...
**Files:**
- Create: `microfactor/eval/execution.py`
- Modify: `microfactor/eval/pipeline.py`
...
from microfactor.eval.domain import EvaluationRun, EvaluationRunConfig, FactorEvaluationResult
from microfactor.eval.execution import SerialEvaluationExecutor
...
from microfactor.eval.calculator import suppress_known_evaluation_warnings
from microfactor.eval.domain import FactorEvaluationResult
from microfactor.eval.evaluator import EvaluationSharedData
...
```bash
git add microfactor/eval/execution.py microfactor/eval/pipeline.py tests/test_eval_execution.py
...
**Files:**
- Create: `microfactor/eval/reporting.py`
- Modify: `microfactor/eval/report.py`
...
from microfactor.eval.domain import EvaluationRun, EvaluationRunConfig
from microfactor.eval.report import ReportThresholds
from microfactor.eval.reporting import (
    EvaluationReporter,
...
Expected: fail because `microfactor.eval.reporting` does not exist.

- [ ] **Step 3: Implement `reporting.py`**

Create `microfactor/eval/reporting.py` by moving the logic currently in
...
Modify `generate_evaluation_report` in `microfactor/eval/report.py` to construct
...
```bash
git add microfactor/eval/reporting.py microfactor/eval/report.py tests/test_eval_reporting.py
...
**Files:**
- Modify: `microfactor/eval/analysis.py`
...
def test_analysis_runner_writes_family_outputs(tmp_path):
    from microfactor.eval.analysis import EvaluationAnalysisRunner
    from microfactor.eval.domain import EvaluationRun, EvaluationRunConfig
...
```bash
git add microfactor/eval/analysis.py tests/test_eval_analysis.py
...
**Files:**
- Create: `microfactor/eval/workflow.py`
- Modify: `microfactor/eval/__init__.py`
...
from microfactor.eval.workflow import EvaluationWorkflow
from microfactor.eval.domain import EvaluationRequest
from microfactor.storage import FactorStorage
...
from microfactor.eval.evaluator import FactorEvaluator
from microfactor.eval.execution import ProcessPoolEvaluationExecutor, SerialEvaluationExecutor
from microfactor.eval.figures import FactorFigureWriter
from microfactor.eval.reporting import (
    EvaluationReporter,
    MarkdownReportRenderer,
...
```bash
git add microfactor/eval/workflow.py microfactor/eval/__init__.py tests/test_eval_workflow.py
...
**Files:**
- Modify: `microfactor/eval/batch.py`
- Modify: `microfactor/services/evaluate.py`
...
```python
from microfactor.eval.domain import EvaluationRequest
from microfactor.services.evaluate import EvaluationService
...
```bash
git add microfactor/eval/batch.py microfactor/services/evaluate.py tests/test_services_evaluate.py
...
**Files:**
- Modify: `microfactor/cli/evaluate_cmds.py`
...
```python
from microfactor.eval import EvaluationRequest
```
...
```bash
git add microfactor/cli/evaluate_cmds.py tests/test_cli.py
...
**Files:**
- Modify: `microfactor/eval/pipeline.py`
- Modify: `microfactor/eval/config.py`
- Modify: `microfactor/eval/__init__.py`
...
Replace `microfactor/eval/pipeline.py` with compatibility wrappers or remove it
...
Modify `microfactor/eval/__init__.py` so the primary exported API is:
...
```bash
git add microfactor/eval/pipeline.py microfactor/eval/config.py microfactor/eval/__init__.py tests
...
```bash
wc -l microfactor/eval/*.py microfactor/eval/metrics/*.py
```
