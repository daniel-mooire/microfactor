...
**Architecture:** 将 `microfactor/eval/metrics.py` 拆为子包 `metrics/`，各指标类型独立模块，`metrics/__init__.py` 保持对外接口不变。`build_summary` 新增可选参数 `clean_factor_data` 和 `index_returns`，有值时追加扩展列，无值时行为与现在完全一致。
...
| 操作 | 路径 |
|---|---|
| 创建 | `microfactor/eval/metrics/__init__.py` |
| 创建 | `microfactor/eval/metrics/ic.py` |
| 创建 | `microfactor/eval/metrics/returns.py` |
| 创建 | `microfactor/eval/metrics/turnover.py` |
| 创建 | `microfactor/eval/metrics/monotonicity.py` |
| 删除 | `microfactor/eval/metrics.py` |
| 修改 | `microfactor/eval/loaders.py` |
| 修改 | `microfactor/eval/config.py` |
| 修改 | `microfactor/eval/pipeline.py` |
| 修改 | `microfactor/eval/report.py` |
| 修改 | `main.py` |
...
**Files:**
- 创建: `microfactor/eval/metrics/__init__.py`
- 创建: `microfactor/eval/metrics/ic.py`（空）
- 创建: `microfactor/eval/metrics/returns.py`（空）
- 创建: `microfactor/eval/metrics/turnover.py`（空）
- 创建: `microfactor/eval/metrics/monotonicity.py`（空）
- 删除: `microfactor/eval/metrics.py`
- 修改: `tests/test_eval_metrics.py`
...
```bash
cd /data/microfactor && uv run pytest tests/test_eval_metrics.py -v
```
...
```bash
mkdir -p microfactor/eval/metrics
cp microfactor/eval/metrics.py microfactor/eval/metrics/__init__.py
```
...
```bash
rm microfactor/eval/metrics.py
```
...
```bash
git add microfactor/eval/metrics/ microfactor/eval/metrics.py tests/test_eval_metrics.py
...
- 创建: `microfactor/eval/metrics/ic.py`
- 创建: `tests/test_eval_metrics_ic.py`
- 修改: `microfactor/eval/metrics/__init__.py`
...
from microfactor.eval.metrics.ic import (
...
def test_build_summary_includes_ic_freq_and_ratio_columns():
    from microfactor.eval.metrics import build_summary
...
- [ ] **Step 3: 实现 `microfactor/eval/metrics/ic.py`**
...
在 `microfactor/eval/metrics/__init__.py` 中，找到 `build_summary` 函数，在最后将新列合并进每行结果：
...
```bash
git add microfactor/eval/metrics/ tests/test_eval_metrics_ic.py
...
from microfactor.eval.metrics.returns import (
    annualized_return,
    max_drawdown,
    sharpe_ratio,
    calmar_ratio,
...
- [ ] **Step 3: 实现 `microfactor/eval/metrics/returns.py`**
...
在 `microfactor/eval/metrics/__init__.py` 顶部新增导入：

```python
from microfactor.eval.metrics.returns import build_group_return_metrics, detect_direction
```
...
```bash
git add microfactor/eval/metrics/ tests/test_eval_metrics_returns.py
...
from microfactor.eval.metrics.turnover import calculate_quantile_turnover
...
- [ ] **Step 3: 实现 `microfactor/eval/metrics/turnover.py`**
...
```bash
git add microfactor/eval/metrics/ tests/test_eval_metrics_turnover.py
...
from microfactor.eval.metrics.monotonicity import (
...
- [ ] **Step 3: 实现 `microfactor/eval/metrics/monotonicity.py`**
...
在 `microfactor/eval/report.py` 的 `_display_columns` 函数中，在 `"monotonicity"` 后追加：
...
```bash
git add microfactor/eval/metrics/ microfactor/eval/report.py tests/test_eval_metrics_monotonicity.py
...
from microfactor.eval.loaders import load_index_daily
...
- [ ] **Step 3: 在 `microfactor/eval/loaders.py` 中添加 `load_index_daily`**
...
```bash
git add microfactor/eval/loaders.py tests/test_eval_loaders.py
...
在 `microfactor/eval/pipeline.py` 的 `evaluate_factor` 函数中，找到 `build_summary(...)` 调用，新增参数：
...
```bash
git add microfactor/eval/config.py microfactor/eval/pipeline.py tests/test_eval_config.py tests/test_eval_pipeline.py
...
def test_build_ranked_summary_includes_return_columns_when_present():
    from microfactor.eval.report import build_ranked_summary, ReportThresholds
...
```bash
git add microfactor/eval/report.py main.py tests/test_eval_report.py
