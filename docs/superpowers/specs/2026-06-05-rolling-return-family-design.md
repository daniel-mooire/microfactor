...
`microfactor` already has four built-in return factors:
...
The resulting raw family has 32 factors. The design should migrate the useful family/profile/pipeline structure from `zer0alpha`, while keeping the `microfactor` decision that rolling return raw factors are derived from already stored base return factors rather than recomputed from `open` and `close` market data.
...
Add `microfactor/factors/rolling_returns.py`.
...
Add a lightweight family pipeline module at `microfactor/pipeline.py`.
