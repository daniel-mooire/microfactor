...
**文件：** `microfactor/registry.py`（独立模块，不依赖 eval 层）
...
| `microfactor/registry.py` | 新增，`FactorMeta`、`EvaluateMeta`、`RegistryValidation`、`FactorRegistry` |
| `microfactor/eval/batch.py` | 修改，支持 `factor_source = "registry"` |
