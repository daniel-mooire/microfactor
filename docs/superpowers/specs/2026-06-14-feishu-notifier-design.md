...
microfactor 的预处理（`build-factors`）和评估（`evaluate-batch`）任务耗时较长，目前运行期间没有任何外部通知。参考 zer0alpha 的企业微信通知实现，在 microfactor 中加入飞书富文本卡片通知。
...
`Config` dataclass（`microfactor/config.py`）增加字段：
...
```json
{
  ...
  "card": {
    "header": {
      "title": {"tag": "plain_text", "content": "[microfactor] `preprocess` 完成 ✅"},
      "template": "green"
    },
    "elements": [
      {
...
### `microfactor/pipeline.py` — `run_build_stage()`
...
### `microfactor/eval/pipeline.py` — batch 评估入口
