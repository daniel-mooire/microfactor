...
Add a standard cross-sectional factor preprocessing pipeline for microfactor.
...
- After processing, long input is converted back to
  `trade_date, ts_code, value` using the existing microfactor schema.
...
This avoids producing false precision before microfactor has reliable exposure
data.
...
- The public API is importable from `microfactor.preprocess`.
