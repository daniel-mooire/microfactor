...
Add total market cap and circulating market cap as first-class microfactor
...
Use `microshare.LocalPro.daily_basic`.
...
Both source values are treated as numeric market cap values from microshare. The
factor layer does not rescale units; it applies natural log consistently to the
source values.

## FactorFrame Fields

Extend `microfactor.core.STANDARD_FIELDS` with:
...
```text
microfactor/factors/market_cap.py
```
...
1. Loads `total_mv` and `circ_mv` through `MicroshareDataProvider`.
...
- If `daily_basic` does not return a requested market cap column, raise
  `ValueError("microshare result missing source column: <column>")`, matching
...
- `MicroshareDataProvider.history()` loads market cap fields from
  `daily_basic`.
