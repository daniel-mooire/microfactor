...
`notebooks/01_alphalens_pct_chg.ipynb` into a reusable microfactor evaluation
pipeline.
...
Add the following modules under `microfactor/eval/`:
...
- `alphalens_adapter.py`: convert microfactor data into Alphalens inputs.
...
Use `microshare.LocalPro.pro_bar` to read prices.
...
- Keep core logic in `microfactor/eval/`.
...
They should not depend on a real microshare data directory.
