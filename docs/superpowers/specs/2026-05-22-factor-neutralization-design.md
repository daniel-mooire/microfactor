...
- Industry exposure: SW L1 industry from `microshare.LocalPro.index_member_all`
...
Add a helper that converts microshare industry membership history into a wide
...
- Missing industry data raises the microshare `FileNotFoundError` with its
  existing message.
