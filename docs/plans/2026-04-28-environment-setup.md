...
**Goal:** Scaffold the microfactor project — git, pyproject.toml, config, storage skeleton, and per-layer init files.

**Architecture:** microfactor uses `LocalPro` from microshare directly (`from microshare.api import LocalPro`). No `loader.py` wrapper needed. Factor storage lives in `data/factors/` with the same Parquet + DuckDB pattern as microshare.

**Tech Stack:** Python 3.11+, uv, microshare (path dep), duckdb, pyarrow, pandas, loguru, click, pytest, ruff
...
Expected: Lockfile created, `.venv` populated, microshare installed from `../microshare`.

**Step 3: Verify microshare import**

```bash
uv run python -c "from microshare.api import LocalPro; print('OK')"
```
...
git commit -m "chore: add pyproject.toml with microshare path dependency"
```
...
""")
    ...
    assert cfg.microshare_data_dir == Path("../microshare/data")
...
```toml
[microshare]
data_dir = "../microshare/data"   # microshare 的 Parquet 数据目录
...
Edit `config/settings.toml` — set `microshare.data_dir` to microshare's actual data path (e.g. `../microshare/data`).
...
- [ ] `uv sync --dev` succeeds, `from microshare.api import LocalPro` works
