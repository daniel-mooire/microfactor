...
```text
-1 * rank(corr(rank(vwap), rank(volume), 5))
```
...
Reverses the cross-sectional rank of five-day VWAP-rank and volume-rank correlation.
...
- Python 实现：`microfactor.factors.alpha191_document::alpha191_090`
- 默认参数：`{"alpha191_number":90,"normalization":"Only case and redundant parentheses were normalized.","normalized_expression":"-1 * rank(corr(rank(vwap), rank(volume), 5))","recursive_stability_window":null,"source_expression":"(RANK(CORR(RANK(VWAP),RANK(VOLUME),5))*-1)","source_fq":null}`
