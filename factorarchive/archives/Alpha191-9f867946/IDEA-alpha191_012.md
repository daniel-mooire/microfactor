...
输入字段：`open, close, vwap`

## 原理说明

Combines the open gap from ten-day VWAP with the magnitude of the close-to-VWAP deviation.
...
- Python 实现：`microfactor.factors.alpha191_document::alpha191_012`
- 默认参数：`{"alpha191_number":12,"normalization":"Only case and redundant parentheses were normalized.","normalized_expression":"rank(open - (sum(vwap, 10) / 10)) * (-1 * rank(abs(close - vwap)))","recursive_stability_window":null,"source_expression":"RANK(OPEN-SUM(VWAP,10)/10)*(-1*RANK(ABS(CLOSE-VWAP)))","source_fq":null}`
