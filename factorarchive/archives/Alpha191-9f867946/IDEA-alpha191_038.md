...
Two-day high-price reversal, active only above the 20-day average high.
...
- Python 实现：`microfactor.factors.alpha191_document::alpha191_038`
- 默认参数：`{"alpha191_number":38,"normalization":"Only case and redundant parentheses were normalized.","normalized_expression":"((sum(high, 20) / 20) < high) ? (-1 * delta(high, 2)) : 0","recursive_stability_window":null,"source_expression":"(((SUM(HIGH,20)/20)<HIGH)?(-1*DELTA(HIGH,2)):0)","source_fq":null}`
