...
Coefficient of variation of absolute return per share volume over twenty days.
...
- Python 实现：`microfactor.factors.alpha191_document::alpha191_076`
- 默认参数：`{"alpha191_number":76,"normalization":"Only case and redundant parentheses were normalized.","normalized_expression":"std(abs((close / delay(close, 1)) - 1) / volume, 20) / mean(abs((close / delay(close, 1)) - 1) / volume, 20)","recursive_stability_window":null,"source_expression":"STD(ABS(CLOSE/DELAY(CLOSE,1)-1)/VOLUME,20)/MEAN(ABS(CLOSE/DELAY(CLOSE,1)-1)/VOLUME,20)","source_fq":null}`
