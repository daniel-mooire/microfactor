...
```text
-1 * max(rank(decaylinear(delta(vwap, 1), 12)), rank(decaylinear(rank(corr(low, mean(volume, 80), 8)), 17)))
```
...
Takes the weaker side of short VWAP movement and a long-volume low-price relation after decay and ranking.
...
- Python 实现：`microfactor.factors.alpha191_document::alpha191_061`
- 默认参数：`{"alpha191_number":61,"normalization":"Only case and redundant parentheses were normalized.","normalized_expression":"-1 * max(rank(decaylinear(delta(vwap, 1), 12)), rank(decaylinear(rank(corr(low, mean(volume, 80), 8)), 17)))","recursive_stability_window":null,"source_expression":"MAX(RANK(DECAYLINEAR(DELTA(VWAP,1),12)),RANK(DECAYLINEAR(RANK(CORR(LOW,MEAN(VOLUME,80),8)),17)))*-1","source_fq":null}`
