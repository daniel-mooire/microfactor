...
```text
-1 * (tsrank(decaylinear(decaylinear(corr(close, volume, 10), 16), 4), 5) - rank(decaylinear(corr(vwap, mean(volume, 30), 4), 3)))
```
...
Contrasts a twice-decayed close/volume relation with a short VWAP/average-volume relation.
...
- Python 实现：`microfactor.factors.alpha191_document::alpha191_073`
- 默认参数：`{"alpha191_number":73,"normalization":"Only case and redundant parentheses were normalized.","normalized_expression":"-1 * (tsrank(decaylinear(decaylinear(corr(close, volume, 10), 16), 4), 5) - rank(decaylinear(corr(vwap, mean(volume, 30), 4), 3)))","recursive_stability_window":null,"source_expression":"(TSRANK(DECAYLINEAR(DECAYLINEAR(CORR(CLOSE,VOLUME,10),16),4),5)-RANK(DECAYLINEAR(CORR(VWAP,MEAN(VOLUME,30),4),3)))*-1","source_fq":null}`
