...
```text
100*SUM(max(0,H-Ref(MID,1)),20)/SUM(max(0,Ref(MID,1)-L),20)
```
...
- Python 实现：`microfactor.factors.document_indicators::doc_cr_20`
- 默认参数：`{}`
- 适配说明：无。
...
- Rank IC Mean：`-0.0484420273011`
- Rank ICIR：`-0.404086285724`
- Adjusted ICIR：`-0.404086285724`
- Long-short spread (bps)：`-24.8760703907`
- Monotonicity：`-0.963636363636`
- Coverage：`0.839984889578`
- Daily turnover (long)：`0.248712465739`
- Daily turnover (short)：`0.238013366642`
