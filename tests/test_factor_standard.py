...
def test_microshare_provider_maps_local_api_to_factor_frame():
    pro = FakeLocalPro()
    ...
    assert frame.volume.loc[pd.Timestamp("2024-01-02"), "000001.SZ"] == 20000


def test_microshare_provider_normalizes_amount_and_vwap_units():
    class VwapLocalPro(FakeLocalPro):
        def pro_bar(self, ts_code, start_date, end_date, adj):
            return pd.DataFrame(
                {
                    "ts_code": ["000001.SZ"],
    ...
    frame = MicroshareDataProvider(VwapLocalPro()).history(
        fields=["volume", "amount", "vwap"],
        start_date="20240101",
        end_date="20240101",
        universe="all",
        adjust="hfq",
    ...
    assert frame.volume.iloc[0, 0] == 10000.0
    assert frame.amount.iloc[0, 0] == 1_000_000.0
    assert frame.vwap.iloc[0, 0] == 100.0
...
def test_microshare_provider_uses_stable_daily_basic_field_order():
    pro = FakeLocalPro()
    provider = MicroshareDataProvider(pro)
...
def test_microshare_provider_loads_turnover_rate_from_daily_basic():
    pro = FakeLocalPro()
...
def test_microshare_provider_returns_empty_panels_for_empty_universe():
