...
def test_build_price_matrix_treats_nonpositive_prices_as_missing():
    raw = pd.DataFrame(
        {
            "trade_date": ["20240101", "20240102"],
            "ts_code": ["000001.SZ", "000001.SZ"],
    ...
    assert pd.isna(result.loc[pd.Timestamp("2024-01-01"), "000001.SZ"])
    assert result.loc[pd.Timestamp("2024-01-02"), "000001.SZ"] == 11.5
...
def test_filter_factor_by_tradability_removes_t1_limit_and_missing_open():
    index = pd.MultiIndex.from_tuples(
        [
            (pd.Timestamp("2024-01-01"), "000001.SZ"),
            (pd.Timestamp("2024-01-01"), "000002.SZ"),
    ...
    diagnostics = pd.DataFrame(
        {
            ...
            "missing_open_t1": [False, True, False],
            "up_limit_t1": [False, False, True],
        },
        index=index,
    )

    result, counts = filter_factor_by_tradability(factor, diagnostics)

    assert list(result) == [1.0]
    assert counts["ineligible_t1"] == 2
    assert counts["eligible_t1"] == 1
