...
def test_reporter_writes_ranked_summary_and_markdown(tmp_path):
    ...
    assert "计算公式" in html
    assert "小白解读" in html
    assert "本次样本中的原理" in html
    assert "完整指标" not in html
    assert "口径与可交易性诊断" not in html
    assert "width:100%;max-width:100%;height:auto" in html
    assert "annual-table" in html
    assert "运行批次" not in html
    assert "因子结果索引" in html
    assert "factors/factor_good/report.html" in html
    assert "factors/factor_bad/report.html" in html
    assert (run.factor_dir("factor_good") / "report.html").exists()
    assert (run.factor_dir("factor_bad") / "report.html").exists()
    factor_html = (run.factor_dir("factor_good") / "report.html").read_text(encoding="utf-8")
    assert "返回汇总" in factor_html
    assert "当前报告" in factor_html
    assert "计算公式" in factor_html
