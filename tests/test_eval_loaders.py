...
def test_load_index_daily_drops_nan_pct_chg_rows():
    ...
    result = load_index_daily(
        NanRowPro(), ts_code="000300.SH", start_date="20240101", end_date="20240105"
    )
    assert len(result) == 2
...
def test_load_index_daily_deduplicates_trade_date():
    ...
    result = load_index_daily(
        DupPro(), ts_code="000300.SH", start_date="20240101", end_date="20240105"
    )
    assert len(result) == 3
