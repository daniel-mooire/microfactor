...
from microfactor.eval.analysis import ANALYSIS_CONFIGS, run_analysis
...
@click.option("--workers", default=None, type=int, help="评估并行进程数")
@click.pass_context
def evaluate_batch_command(ctx, batch_file, benchmark_index, workers):
    ...
    if getattr(report, "html_path", None) is not None:
        click.echo(f"HTML report written to {report.html_path}")
...
def evaluate_summary_command(
    ...
    if getattr(result, "html_path", None) is not None:
        click.echo(f"HTML report written to {result.html_path}")
