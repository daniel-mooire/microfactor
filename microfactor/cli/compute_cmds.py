...
from microfactor.factor_registry import FAMILIES, FactorFamilyRegistry
from microfactor.factors.builtin import MARKET_CAP_FACTORS, RETURN_FACTORS
from microfactor.factors.catalog import get_catalog_factor
...
@cli.command("compute-catalog-factor")
@click.argument("factor_name")
@click.option("--start-date", default=None)
@click.option("--end-date", default=None)
@click.option("--universe", default=None)
@click.pass_context
def compute_catalog_factor_command(ctx, factor_name, start_date, end_date, universe):
    """Compute one copied catalog factor and persist its quality manifest."""
    cfg = load_config(ctx.obj["config_path"])
    app = AppContext(cfg)
    factor = get_catalog_factor(factor_name)
    resolved_start = start_date or cfg.start_date
    resolved_end = end_date if end_date is not None else (cfg.end_date or None)
    resolved_universe = universe or cfg.universe
    ...
    click.echo(f"{factor_name}: {counts[factor_name]} rows")


@cli.command("compute-catalog")
@click.option("--start-date", default=None)
@click.option("--end-date", default=None)
@click.option("--universe", default=None)
@click.option("--resume/--no-resume", default=False, show_default=True)
@click.pass_context
def compute_catalog_command(ctx, start_date, end_date, universe, resume):
    """Compute all copied non-model catalog factors with durable manifests."""
    cfg = load_config(ctx.obj["config_path"])
    app = AppContext(cfg)
    ...
    resolved_end = end_date if end_date is not None else (cfg.end_date or None)
    resolved_universe = universe or cfg.universe
    results = compute_catalog_factors(
        app.provider,
        app.storage,
        start_date=resolved_start,
        end_date=resolved_end,
    ...
    counts = {status: sum(1 for value in results.values() if value["status"] == status)
              for status in {str(value["status"]) for value in results.values()}}
    for factor_name in sorted(results):
        item = results[factor_name]
        suffix = f" error={item['error']}" if item.get("error") else ""
        click.echo(f"{factor_name}: {item['status']} rows={item.get('rows', 0)}{suffix}")
    click.echo(f"catalog factors={len(results)} statuses={counts}")
