...
def load_config(path: Path = Path("config/settings.toml")) -> Config:
    ...
    try:
        ...
        # TOML basic strings treat Windows backslashes as escapes.  Existing
        # project configs commonly contain native Windows paths, so retry with
        # normalized separators while preserving the parsed values.
