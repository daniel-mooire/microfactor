...
class MicroshareDataProvider:
    """Adapter from microshare LocalPro into the decoupled FactorFrame contract."""

    # Microshare mirrors the Tushare stock schema: volume is reported in lots
    # and amount in thousand CNY.  Factor code consumes shares and CNY so that
    # ``vwap`` has the same unit as prices.
    _VOLUME_LOT_SIZE = 100.0
    _AMOUNT_THOUSAND_CNY = 1000.0
    ...
    def history(
        ...
        codes = (
            sorted(membership["ts_code"].unique())
            if membership is not None
            else self._resolve_universe(universe)
        )
        total = len(codes)
        ...
        bar_fields = [
            ...
            if field in self._BAR_SOURCE_COLUMNS or field in derived_bar_fields
        ]
        daily_basic_fields = [
        ...
        if daily_basic_fields and codes:
            daily_basic_source_fields = [
                source
                for source in self._DAILY_BASIC_FIELD_ORDER
                if source in {
        ...
        for field_name in requested:
            if field_name in self._BAR_SOURCE_COLUMNS:
                panel = self._pivot_field(bar_raw, self._BAR_SOURCE_COLUMNS[field_name])
                if field_name == "volume":
                    panel = panel * self._VOLUME_LOT_SIZE
                elif field_name == "amount":
                    panel = panel * self._AMOUNT_THOUSAND_CNY
                panels[field_name] = panel
            elif field_name in self._DAILY_BASIC_SOURCE_COLUMNS:
                ...
                adjusted = self._pivot_field(bar_raw, "close")
                panels["adjusted_close"] = adjusted
                if adjusted.empty:
                    total_return_index = adjusted.copy()
                else:
                    first_valid = adjusted.bfill().iloc[0].replace(0, pd.NA)
                    total_return_index = adjusted.div(first_valid, axis="columns")
                panels["close_total_return_index"] = total_return_index
                panels["close_to_close_total_return_1d"] = panels[
                    "close_total_return_index"
                ].pct_change(fill_method=None)
            elif field_name == "vwap":
                amount = self._pivot_field(bar_raw, "amount")
                volume = self._pivot_field(bar_raw, "vol")
                panels[field_name] = (amount * self._AMOUNT_THOUSAND_CNY).div(
                    (volume * self._VOLUME_LOT_SIZE).where(volume != 0)
                )
            elif field_name in {"is_suspended", "is_st"}:
                panels[field_name] = self._load_flag_panel(
                    field_name, codes, start_date, end_date
                )
            elif field_name in {"up_limit", "down_limit"}:
                limits = self._pro.stk_limit(
                    ts_code=",".join(codes), start_date=start_date, end_date=end_date
                )
                panels[field_name] = self._pivot_field(limits, field_name)
            elif field_name == "list_date":
                basic = self._pro.stock_basic(
                    ts_code=",".join(codes), fields="ts_code,list_date"
                )
                panels[field_name] = self._static_panel(basic, "list_date", start_date, end_date)
            else:
                raise ValueError(f"unsupported Microshare field: {field_name}")
        membership_panel = self._membership_panel(membership, panels)
        if membership_panel is not None:
            panels = {
                name: frame.reindex(index=membership_panel.index, columns=membership_panel.columns)
                .where(membership_panel)
    ...
    def _load_membership(
        ...
        *,
        ...
        if not isinstance(universe, str) or universe == "all" or "," in universe:
            return None
        rows = self._pro.universe(
            universe=universe,
            start_date=start_date,
            end_date=end_date,
        ...
        if rows.empty:
            raise ValueError(f"Microshare universe returned no rows: {universe}")
        result = rows.loc[:, ["trade_date", "ts_code"]].dropna().drop_duplicates()
    ...
    def _membership_panel(
        ...
        if membership is None:
            return None
        index = pd.DatetimeIndex(sorted(set(membership["trade_date"])))
        columns = pd.Index(sorted(membership["ts_code"].unique()), dtype="object")
        keys = membership.drop_duplicates()
        return (
            keys.assign(value=True)
            .pivot(index="trade_date", columns="ts_code", values="value")
            .reindex(index=index, columns=columns, fill_value=False)
            .astype("boolean")
            .fillna(False)
            .astype(bool)
        )

    def _load_flag_panel(
        self, field: str, codes: list[str], start_date: str, end_date: str | None
    ) -> pd.DataFrame:
        if field == "is_st":
            raw = self._pro.stock_st(
                ts_code=",".join(codes), start_date=start_date, end_date=end_date
            )
        else:
            raw = self._pro.suspend_d(
                ts_code=",".join(codes), start_date=start_date, end_date=end_date
            )
        if raw.empty:
            dates = pd.date_range(
                pd.to_datetime(start_date, format="%Y%m%d"),
                pd.to_datetime(end_date or start_date, format="%Y%m%d"),
                freq="D",
            )
            return pd.DataFrame(False, index=dates, columns=codes, dtype=bool)
        raw = raw.loc[:, ["trade_date", "ts_code"]].drop_duplicates()
        raw["trade_date"] = pd.to_datetime(raw["trade_date"], format="%Y%m%d")
        raw["value"] = True
        panel = raw.pivot(index="trade_date", columns="ts_code", values="value")
        return (
            panel.astype("boolean")
            .fillna(False)
            .astype(bool)
            .sort_index()
            .sort_index(axis=1)
    ...
    def _static_panel(
        ...
        values = raw.set_index("ts_code")[source_column]
        return pd.DataFrame(
            {code: values.get(code) for code in values.index}, index=dates
        )
    ...
    def _pivot_field(raw: pd.DataFrame, source_column: str) -> pd.DataFrame:
        if source_column not in raw.columns:
            raise ValueError(f"microshare result missing source column: {source_column}")
