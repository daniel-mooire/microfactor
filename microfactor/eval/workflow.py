...
from microfactor.eval.execution import (
...
class EvaluationWorkflow:
    ...
    def run(
        ...
        *,
        ...
        # The acceptance flag is added by the report ranker. Keep the run-level
        # manifest and registry aligned with that final decision when a report
        # is generated; otherwise preserve the raw summary for report-free runs.
        decision_summary = report.ranked_summary if report is not None else summary
        self.artifact_store.write_run_manifest(run, decision_summary)
        storage = getattr(self.data_loader, "storage", None)
        if storage is not None and hasattr(storage, "register_evaluation"):
            for factor_result in factor_results:
                rows = decision_summary.loc[
                    decision_summary["factor_name"].astype(str)
                    == factor_result.factor_name
                ]
                ...
                storage.register_evaluation(
                    ...
                    status,
                    metrics=metrics,
                    result_path=factor_result.output_dir,
                    error=error,
                    factor_version=storage.latest_version_id(factor_result.factor_name),
                    evaluation_config={
                    ...
                    report_path=factor_result.output_dir / "report.html",
                )

            failed_count = sum(
                str(row.get("status", "")) in {"failed", "insufficient_data"}
            ...
            storage.register_evaluation_run(
                ...
                completed_count=len(factor_results) - failed_count,
                config={
                    "universe": config.universe,
                    "start_date": config.start_date,
                    "end_date": config.end_date,
