...
from microfactor.eval.metrics import (
    build_summary,
    calculate_daily_ic,
    calculate_five_bucket_returns,
    calculate_quantile_returns,
    calculate_quantile_returns_by_date,
)
...
class MetricsCalculator:
    ...
    def calculate_quantile_returns_by_date(self, clean_factor_data: pd.DataFrame) -> pd.DataFrame:
        with suppress_known_evaluation_warnings(), redirect_stdout(io.StringIO()):
            return calculate_quantile_returns_by_date(clean_factor_data)

    def calculate_five_bucket_returns(self, clean_factor_data: pd.DataFrame) -> pd.DataFrame:
        with suppress_known_evaluation_warnings(), redirect_stdout(io.StringIO()):
            return calculate_five_bucket_returns(clean_factor_data)
