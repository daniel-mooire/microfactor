...
def find_latest_run_dir(evaluations_dir: Path | str = Path("data/evaluations")) -> Path:
    ...
    run_dirs = sorted(
        path for path in root.iterdir() if path.is_dir() and path.name[:1].isdigit()
    )
    if not run_dirs:
...
def generate_evaluation_report(
    *,
    ...
    factor_names = tuple(metadata.get("factor_names") or summary["factor_name"].astype(str))
    summary_start = (
        summary["start_date"].min()
        if "start_date" in summary.columns and not summary["start_date"].dropna().empty
        else "unknown"
    )
    config = EvaluationRunConfig(
        factor_names=factor_names,
        start_date=str(metadata.get("start_date") or summary_start),
        end_date=metadata.get("end_date"),
        periods=tuple(metadata.get("periods") or (1,)),
        quantiles=int(metadata.get("quantiles", 10)),
        return_type=str(metadata.get("return_type", "open_t1")),
        max_loss=float(metadata.get("max_loss", 0.35)),
        universe=metadata.get("universe"),
        output_dir=resolved_run_dir.parent,
        rolling_ic_window=int(metadata.get("rolling_ic_window", 63)),
        transaction_cost_bps=float(metadata.get("transaction_cost_bps", 10.0)),
    )
    ...
    return reporter.generate(_ReportRun(resolved_run_dir, config), summary)
