...
def calculate_quantile_returns_by_date(clean_factor_data: pd.DataFrame) -> pd.DataFrame:
    """Return date-by-quantile forward returns for charts and audit artifacts."""
    mean_returns, _ = mean_return_by_quantile(
        clean_factor_data,
        by_date=True,
        demeaned=False,
    )
    if isinstance(mean_returns.index, pd.MultiIndex):
        names = list(mean_returns.index.names)
        if "date" in names and "factor_quantile" in names:
            return mean_returns.unstack("factor_quantile").sort_index()
    return mean_returns


def calculate_five_bucket_returns(clean_factor_data: pd.DataFrame) -> pd.DataFrame:
    """Build a skill-compatible five-bucket view from the cleaned panel."""
    required = {"factor_quantile", "1D"}
    missing = required.difference(clean_factor_data.columns)
    if missing:
        return pd.DataFrame()

    valid = clean_factor_data[["factor_quantile", "1D"]].dropna().copy()
    if valid.empty:
        return pd.DataFrame()
    # Match factor-eval's equal-weight excess convention by removing the
    # cross-sectional mean return for each signal date.
    dates = valid.index.get_level_values("date")
    valid["return"] = valid["1D"] - valid.groupby(dates)["1D"].transform("mean")
    frame = pd.DataFrame(
        {
            "date": dates,
            "bucket": ((valid["factor_quantile"].to_numpy(dtype=int) - 1) // 2) + 1,
            "return": valid["return"].to_numpy(),
        }
    )
    return frame.groupby(["date", "bucket"])["return"].mean().unstack("bucket")
...
def build_portfolio_returns(
    ...
    *,
    ...
    turnover_long: float = 0.0,
    turnover_short: float = 0.0,
) -> pd.DataFrame:
    """Build gross/net long-short daily returns from quantile membership."""
    by_date = calculate_quantile_returns_by_date(clean_factor_data)
    if by_date.empty or by_date.shape[1] < 2:
        return pd.DataFrame()
    if isinstance(by_date.columns, pd.MultiIndex):
        try:
            by_date = by_date.xs("1D", axis=1, level=0)
        except KeyError:
            return pd.DataFrame()
    elif "1D" in by_date.columns:
        by_date = by_date[["1D"]]
    else:
        return pd.DataFrame()
    low, high = by_date.columns.min(), by_date.columns.max()
    result = pd.DataFrame(index=by_date.index)
    result["gross_long"] = by_date[high if direction > 0 else low]
    result["gross_short"] = by_date[low if direction > 0 else high]
    result["gross_long_short"] = (
        (result["gross_long"] - result["gross_short"]) * direction
    )

    result["turnover_long"] = float(turnover_long)
    result["turnover_short"] = float(turnover_short)

    cost_rate = transaction_cost_bps / 10000.0
    total_cost = (result["turnover_long"] + result["turnover_short"]) * cost_rate
    result["net_long_short"] = result["gross_long_short"] - total_cost
    return result
